-- Temporary Test Migration: 013_reviewer_decision_test_rpc.sql
-- Purpose: Safely test transaction rollback for Reviewer Final Decisions

CREATE OR REPLACE FUNCTION process_reviewer_decision_test(
  p_identity_id UUID,
  p_action TEXT,
...
BEGIN
  ...
  IF p_action = 'REJECT' THEN
    ...
    -- DELIBERATE EXCEPTION FOR ROLLBACK TEST
    RAISE EXCEPTION 'Simulated Rollback Failure (Reject path)';
  ...
  ELSIF p_action = 'APPROVE' THEN
    ...
    -- DELIBERATE EXCEPTION FOR ROLLBACK TEST
    RAISE EXCEPTION 'Simulated Rollback Failure (Approve path)';
