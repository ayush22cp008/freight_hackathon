...
export async function POST(request: Request) {
  try {
    ...
    // Verify required preceding milestone (GOODS_UNLOADED)
    ...
    if (!unloadedEvent) {
      return NextResponse.json({ error: 'Cannot record delivery departed before goods unloaded is complete' }, { status: 400 });
    }
    ...
    if (error) {
      ...
      if (error.code === '23505') {
        return NextResponse.json({ error: 'Delivery departed already recorded for this trip' }, { status: 409 });
      }
      return NextResponse.json({ error: 'Failed to record delivery departed' }, { status: 500 });
    }
