...
export async function POST(request: Request) {
  try {
    ...
    // Verify required preceding milestone (checkin or PICKUP_CHECKED_IN)
    ...
    if (!checkinEvent) {
      return NextResponse.json({ error: 'Cannot load goods before check-in is complete' }, { status: 400 });
    }
    ...
    if (error) {
      ...
      if (error.code === '23505') {
        return NextResponse.json({ error: 'Goods loaded already recorded for this trip' }, { status: 409 });
      }
      return NextResponse.json({ error: 'Failed to record load' }, { status: 500 });
    }
