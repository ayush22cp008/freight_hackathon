...
export async function POST(request: Request) {
  try {
    ...
    // Verify required preceding milestone (GOODS_LOADED)
    ...
    if (!loadEvent) {
      return NextResponse.json({ error: 'Cannot record pickup departure before goods are loaded' }, { status: 400 });
    }
    ...
    if (error) {
      ...
      if (error.code === '23505') {
        return NextResponse.json({ error: 'Pickup departure already recorded for this trip' }, { status: 409 });
      }
      return NextResponse.json({ error: 'Failed to record pickup departure' }, { status: 500 });
    }
