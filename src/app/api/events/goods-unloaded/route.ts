...
export async function POST(request: Request) {
  try {
    ...
    // Verify required preceding milestone (RECEIVER_CHECKED_IN)
    ...
    if (!receiverEvent) {
      return NextResponse.json({ error: 'Cannot record goods unloaded before receiver check-in is complete' }, { status: 400 });
    }
    ...
    if (error) {
      ...
      if (error.code === '23505') {
        return NextResponse.json({ error: 'Goods unloaded already recorded for this trip' }, { status: 409 });
      }
      return NextResponse.json({ error: 'Failed to record goods unloaded' }, { status: 500 });
    }
