...
export async function POST(request: Request) {
  try {
    ...
    // Verify required preceding milestone (PICKUP_DEPARTED or legacy departure)
    ...
    if (!departedEvent) {
      return NextResponse.json({ error: 'Cannot record transit before pickup departure is complete' }, { status: 400 });
    }
    ...
    if (error) {
      ...
      if (error.code === '23505') {
        return NextResponse.json({ error: 'Transit already recorded for this trip' }, { status: 409 });
      }
      return NextResponse.json({ error: 'Failed to record transit' }, { status: 500 });
    }
