...
export async function POST(request: Request) {
  try {
    ...
    // 1. Atomically record the receiver's confirmation if not already set
    const { error: updateError } = await supabaseServer
      .from('trips')
      .update({ receiver_delivery_confirmed_at: new Date().toISOString() })
      .eq('id', trip_id)
      .is('receiver_delivery_confirmed_at', null);

    if (updateError) {
      console.error('Failed to set receiver confirmation:', updateError);
