...
export default function ReceiverCheckinClient({ tripId, destinationName, driverName }: { tripId: string; destinationName: string; driverName: string }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 4. Submit to API
      const res = await fetch('/api/events/receiver-checkin', {
        method: 'POST',
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Failed to submit receiver check-in event');
      }
    ...
    return (
      <div className="p-8 max-w-2xl mx-auto space-y-4">
        <div className="bg-green-100 text-green-800 p-6 rounded-lg shadow border border-green-200">
          <h2 className="text-2xl font-bold mb-2">Receiver Checked In Successfully!</h2>
          <p><strong>Timestamp:</strong> {new Date(success.timestamp).toLocaleString()}</p>
          {success.photo && (
            /* eslint-disable-next-line @next/next/no-img-element */
            <img src={success.photo} alt="Receiver proof" className="mt-4 max-w-sm rounded shadow-sm border border-gray-200" />
          )}
          <button 
            onClick={() => router.push('/')}
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto space-y-6">
      <h1 className="text-2xl font-bold text-gray-900">Check In Receiver: {destinationName}</h1>
      
      <div className="bg-white p-6 rounded-lg shadow space-y-4 border border-gray-200">
        <div>
          <p className="font-semibold text-gray-700">Driver: <span className="font-normal">{driverName}</span></p>
          <p className="text-sm text-gray-600">Please confirm receipt of the delivery by providing your check-in evidence below.</p>
        </div>
        ...
        >
          {loading ? 'Recording...' : 'Submit Receiver Check-In'}
        </button>
