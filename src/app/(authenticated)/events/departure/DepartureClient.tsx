...
export default function DepartureClient({ tripId, facilityName }: { tripId: string; facilityName: string }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Failed to submit departure');
      }
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto space-y-6">
      <h1 className="text-2xl font-bold text-gray-900">Record Departure: {facilityName}</h1>
      
      <div className="bg-white p-6 rounded-lg shadow space-y-4 border border-gray-200">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Proof of Departure (Photo Required)
          </label>
          <input 
            type="file" 
            accept="image/*" 
        ...
        >
          {loading ? 'Recording Departure...' : 'Submit Departure'}
        </button>
