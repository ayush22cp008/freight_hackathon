...
import ArrivedAtDeliveryClient from './ArrivedAtDeliveryClient';

export default async function ArrivedAtDeliveryPage() {
  ...
  // Verify that In Transit occurred
  ...
  if (!inTransitEvent) {
    return (
      <div className="p-8 max-w-2xl mx-auto">
        ...
        <p>You must record In-Transit before recording Arrival at Delivery.</p>
      </div>
    );
  }

  // Check if Arrived At Delivery already exists
  ...
  return <ArrivedAtDeliveryClient tripId={trip.id} facilityName={trip.facility_name || 'Delivery Facility'} />;
}
