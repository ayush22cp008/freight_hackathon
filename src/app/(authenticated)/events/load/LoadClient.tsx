...
export default function LoadClient({ tripId, facilityName }: { tripId: string; facilityName: string }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Failed to submit load event');
      }
    ...
    return (
      <div className="p-8 max-w-2xl mx-auto space-y-4">
        <div className="bg-green-100 text-green-800 p-6 rounded-lg shadow border border-green-200">
          <h2 className="text-2xl font-bold mb-2">Goods Loaded Recorded!</h2>
          <p><strong>Timestamp:</strong> {new Date(success.timestamp).toLocaleString()}</p>
          {success.photo && (
            /* eslint-disable-next-line @next/next/no-img-element */
            <img src={success.photo} alt="Goods loaded proof" className="mt-4 w-full max-w-sm rounded shadow-sm border border-gray-200" />
          )}
          <button 
            onClick={() => router.push('/')}
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto space-y-6">
      <h1 className="text-2xl font-bold text-gray-900">Record Goods Loaded: {facilityName}</h1>
      
      <div className="bg-white p-6 rounded-lg shadow space-y-4 border border-gray-200">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Proof of Goods Loaded (Optional Photo)
          </label>
          <input 
            type="file" 
            accept="image/*" 
        ...
        >
          {loading ? 'Recording...' : 'Submit Goods Loaded'}
        </button>
