...
export default function PickupDepartedClient({ tripId, facilityName }: { tripId: string; facilityName: string }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 4. Submit to API
      const res = await fetch('/api/events/pickup-departed', {
        method: 'POST',
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Failed to submit pickup departure event');
      }
    ...
    return (
      <div className="p-8 max-w-2xl mx-auto space-y-4">
        <div className="bg-green-100 text-green-800 p-6 rounded-lg shadow border border-green-200">
          <h2 className="text-2xl font-bold mb-2">Pickup Departure Recorded!</h2>
          <p><strong>Timestamp:</strong> {new Date(success.timestamp).toLocaleString()}</p>
          {success.photo && (
            /* eslint-disable-next-line @next/next/no-img-element */
            <img src={success.photo} alt="Pickup departure proof" className="mt-4 w-full max-w-sm rounded shadow-sm border border-gray-200" />
          )}
          <button 
            onClick={() => router.push('/')}
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto space-y-6">
      <h1 className="text-2xl font-bold text-gray-900">Record Pickup Departure: {facilityName}</h1>
      
      <div className="bg-white p-6 rounded-lg shadow space-y-4 border border-gray-200">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Proof of Departure (Optional Photo)
          </label>
          <input 
            type="file" 
            accept="image/*" 
        ...
        >
          {loading ? 'Recording...' : 'Submit Pickup Departure'}
        </button>
