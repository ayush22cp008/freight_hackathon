...
export default function GoodsUnloadedClient({ tripId, destinationName }: { tripId: string; destinationName: string }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 4. Submit to API
      const res = await fetch('/api/events/goods-unloaded', {
        method: 'POST',
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Failed to submit goods unloaded event');
      }
    ...
    return (
      <div className="p-8 max-w-2xl mx-auto space-y-4">
        <div className="bg-green-100 text-green-800 p-6 rounded-lg shadow border border-green-200">
          <h2 className="text-2xl font-bold mb-2">Goods Unloaded Successfully!</h2>
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto space-y-6">
      <h1 className="text-2xl font-bold text-gray-900">Record Goods Unloaded: {destinationName}</h1>
      
      <div className="bg-white p-6 rounded-lg shadow space-y-4 border border-gray-200">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Proof of Unloading (Optional Photo)
          </label>
          <input 
            type="file" 
            accept="image/*" 
        ...
        >
          {loading ? 'Recording...' : 'Submit Goods Unloaded'}
        </button>
